...
func TestForm_Valid(t *testing.T) {
	...
	isValid := form.Valid()
	if !isValid {
		t.Error("got invalid when hsould have been valid")
	}
...
func TestForm_Has(t *testing.T) {
	...
	form := New(nil)
	if form.Has("a", r) {
		t.Error("shows has the field when field missing")
	}

	// Test form contains fields
	postedData := url.Values{}
	postedData.Add("a", "a")
	r.PostForm = postedData
	form = New(r.PostForm)
	if !form.Has("a", r) {
		t.Error("shows does not have the field when it does")
	}
}
